define('./workbox-030153e1.js', ['exports'], function (t) {
  'use strict';
  try {
  ...
  class y {
    constructor(t, e, { onupgradeneeded: s, onversionchange: n } = {}) {
      (this.h = null), (this.u = t), (this.l = e), (this.p = s), (this.m = n || (() => this.close()));
    }
    get db() {
      return this.h;
    }
      ...
      if (!this.h)
        return (
          (this.h = await new Promise((t, e) => {
            ...
            (n.onerror = () => e(n.error)),
              (n.onupgradeneeded = (t) => {
                s ? (n.transaction.abort(), n.result.close()) : 'function' == typeof this.p && this.p(t);
              }),
              (n.onsuccess = () => {
                const e = n.result;
  ...
  class v {
    constructor(t) {
      (this.R = t), (this.h = new y('workbox-expiration', 1, { onupgradeneeded: (t) => this.v(t) }));
    }
    v(t) {
    ...
    async setTimestamp(t, e) {
      const s = { url: (t = R(t)), timestamp: e, cacheName: this.R, id: this.q(t) };
    ...
    async expireEntries(t, e) {
      const s = await this.h.transaction(g, 'readwrite', (s, n) => {
          const i = s.objectStore(g).index('timestamp').openCursor(null, 'prev'),
            r = [];
          let a = 0;
          i.onsuccess = () => {
            const s = i.result;
            if (s) {
              const n = s.value;
              n.cacheName === this.R && ((t && n.timestamp < t) || (e && a >= e) ? r.push(s.value) : a++), s.continue();
            } else n(r);
          };
        }),
        n = [];
  ...
  class q {
    ...
    async expireEntries() {
      ...
      const t = this.N ? Date.now() - 1e3 * this.N : 0,
        e = await this.D.expireEntries(t, this.L),
    ...
    async updateTimestamp(t) {
      await this.D.setTimestamp(t, Date.now());
    }
    async isURLExpired(t) {
      if (this.N) {
        return (await this.D.getTimestamp(t)) < Date.now() - 1e3 * this.N;
      }
      return !1;
    }
    async delete() {
  ...
  class C {
    constructor(t, e) {
      (this.T = {}),
        ...
        (this.O = [...t.plugins]),
        (this.W = new Map());
      for (const t of this.O) this.W.set(t, {});
    ...
    async cachePut(t, e) {
      ...
      if (!c) return !1;
      const { cacheName: o, matchOptions: h } = this.P,
        ...
        f = l
          ? await (async function (t, e, s, n) {
              ...
              const r = x({}, n, { ignoreSearch: !0 }),
                a = await t.keys(e, r);
              for (const e of a) if (i === L(e.url, s)) return t.match(e, n);
    ...
    async getCacheKey(t, e) {
      if (!this.T[e]) {
        ...
        for (const t of this.iterateCallbacks('cacheKeyWillBeUsed'))
          s = N(await t({ mode: e, request: s, event: this.event, params: this.params }));
        this.T[e] = s;
      }
      return this.T[e];
    }
  ...
  class E {
    ...
    async A(t, e, n) {
      ...
      try {
        if (((i = await this.I(e, t)), !i || 'error' === i.type)) throw new s('no-response', { url: e.url });
      } catch (s) {
  ...
  class A {
    ...
    createHandlerBoundToURL(t) {
      ...
      return (s) => ((s.request = new Request(t)), (s.params = x({ cacheKey: e }, s.params)), this.strategy.handle(s));
    }
  }
  const S = () => (O || (O = new A()), O);
  class I extends i {
